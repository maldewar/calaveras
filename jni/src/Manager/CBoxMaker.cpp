#include "../Model/CArea.h"
#include "../Model/CUnit.h"
#include "../Engine/AreaCatalog.h"
#include "CBoxMaker.h"

short CBoxMaker::CATEGORY_SCENE = 0x0001;
short CBoxMaker::CATEGORY_UNIT = 0x0002;
short CBoxMaker::MASK_SCENE = -1;
short CBoxMaker::MASK_UNIT = CATEGORY_SCENE;

CBoxMaker::CBoxMaker() {
}

b2Body* CBoxMaker::MakeArea(CArea* area, b2World* bWorld) {
    AreaCatalog::Set(area);
    b2BodyDef bodyDef;
    bodyDef.type = b2_staticBody;
    bodyDef.position.Set(0,0);
    bodyDef.angularDamping = area->GetAngularDamping();
    bodyDef.linearDamping = area->GetLinearDamping();

    b2Body* areaBody = bWorld->CreateBody(&bodyDef);
    areaBody->SetUserData(area);

    b2ChainShape bodyShape;
    b2Vec2 vs[area->GetTotalPoints()];
    int pointIndex = 0;
    for(auto &point : area->GetPoints()) {
        vs[pointIndex].Set(point->x, point->y);
        pointIndex++;
    }
    if (area->IsClosed())
        bodyShape.CreateLoop(vs, area->GetTotalPoints());
    else
        bodyShape.CreateChain(vs, area->GetTotalPoints());

    b2FixtureDef bodyFixtureDef;
    bodyFixtureDef.filter.categoryBits = CATEGORY_SCENE;
    bodyFixtureDef.filter.maskBits = MASK_SCENE;
    bodyFixtureDef.shape = &bodyShape;
    bodyFixtureDef.density = 20.0f;
    bodyFixtureDef.friction = 0.7f;
    b2Fixture* bodyFixture = areaBody->CreateFixture(&bodyFixtureDef);
    bodyFixture->SetUserData((void*)AREA_SENSOR_BODY);

    return areaBody;
}

b2Body* CBoxMaker::MakeUnit(CUnit* unit, b2World* bWorld) {
    b2BodyDef bodyDef;
    bodyDef.type = b2_dynamicBody;
    bodyDef.position.Set(unit->GetX(),unit->GetY());
    bodyDef.angularDamping = 3.0f;
    bodyDef.linearDamping = 1.0f;
    unit->SetOriginalLinearDamping(1.0f);
    unit->SetOriginalAngularDamping(3.0f);
    //bodyDef.fixedRotation = true;

    b2Body* unitBody = bWorld->CreateBody(&bodyDef);
    unitBody->SetUserData(unit);

    b2CircleShape bodyShape;
    bodyShape.m_radius = unit->GetWidth()/2;

    b2FixtureDef bodyFixtureDef;
    bodyFixtureDef.filter.categoryBits = CATEGORY_UNIT;
    bodyFixtureDef.filter.maskBits = MASK_UNIT;
    bodyFixtureDef.shape = &bodyShape;
    bodyFixtureDef.density = 20.0f;
    bodyFixtureDef.friction = 0.3f;
    bodyFixtureDef.restitution = 0.2f;
    b2Fixture* bodyFixture = unitBody->CreateFixture(&bodyFixtureDef);
    bodyFixture->SetUserData((void*)UNIT_SENSOR_BODY);

    float cardinalSensorSize = 0.005;
    float sensorPosition = unit->GetWidth()/2 - cardinalSensorSize/2;

    b2PolygonShape cardinalShape;
    bodyFixtureDef.isSensor = true;
    bodyFixtureDef.shape = &cardinalShape;
    bodyFixtureDef.density = 0.0f;

    cardinalShape.SetAsBox(cardinalSensorSize, cardinalSensorSize, b2Vec2(0,-sensorPosition), M_PI_4);
    b2Fixture* footSensorFixture = unitBody->CreateFixture(&bodyFixtureDef);
    footSensorFixture->SetUserData((void*)UNIT_SENSOR_FEET);

    cardinalShape.SetAsBox(cardinalSensorSize, cardinalSensorSize, b2Vec2(-sensorPosition, 0), M_PI_4);
    b2Fixture* leftHandSensorFixture = unitBody->CreateFixture(&bodyFixtureDef);
    leftHandSensorFixture->SetUserData((void*)UNIT_SENSOR_LEFT_HAND);

    cardinalShape.SetAsBox(cardinalSensorSize, cardinalSensorSize, b2Vec2(sensorPosition, 0), M_PI_4);
    b2Fixture* rightHandSensorFixture = unitBody->CreateFixture(&bodyFixtureDef);
    rightHandSensorFixture->SetUserData((void*)UNIT_SENSOR_RIGHT_HAND);

    cardinalShape.SetAsBox(cardinalSensorSize, cardinalSensorSize, b2Vec2(0, sensorPosition), M_PI_4);
    b2Fixture* headSensorFixture = unitBody->CreateFixture(&bodyFixtureDef);
    headSensorFixture->SetUserData((void*)UNIT_SENSOR_HEAD);

    return unitBody;
}
